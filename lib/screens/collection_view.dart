...
class CollectionView extends StatelessWidget {
    ...
    return Scaffold(
      ...
      body: Consumer<BujoProvider>(
        builder: (context, provider, _) {
          final bullets = collectionId == null 
              ? provider.getInboxBullets() 
              : provider.getCollectionBullets(collectionId!);
  ...
  void _showRenameDialog(BuildContext context) {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        title: const Text("重命名集子"),
        content: TextField(controller: controller, autofocus: true),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx), child: const Text("取消")),
          ElevatedButton(
            onPressed: () {
              if (controller.text.isNotEmpty && collectionId != null) {
  ...
  void _showDeleteCollectionDialog(BuildContext context) {
    ...
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text("删除集子"),
        content: const Text("确定要删除这个集子吗？"),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx), child: const Text("取消")),
          TextButton(
            onPressed: () {
