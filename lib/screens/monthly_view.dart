...
class _MonthlyViewState extends State<MonthlyView> {
  ...
  void initState() {
    ...
    // 计算焦点月份与锚点月份的差值 (年差*12 + 月差)
    // 这样才能正确跳转到对应的 Page
    int monthDiff = (_displayedMonth.year - _anchorDate.year) * 12 + 
                    (_displayedMonth.month - _anchorDate.month);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text("$year年 $month月", style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
        centerTitle: true,
      ...
      body: PageView.builder(
        ...
        onPageChanged: (index) {
          setState(() {
            // 根据偏移量计算新月份
            _displayedMonth = DateTime(_anchorDate.year, _anchorDate.month + (index - _anchorIndex), 1);
          });
          ...
          builder: (context, provider, _) {
            ...
            return Container(
              ...
              child: Column(
                ...
                children: [
                  if (monthlyBullets.isNotEmpty)
                    ...monthlyBullets.map((b) => _buildDraggablePoolStrip(b)),
                  if (isHovering && monthlyBullets.isEmpty)
                     const Center(child: Text("放回到本月任务", style: TextStyle(color: Colors.blue, fontSize: 12))),
                ],
              ),
            );
          },
        ...
        return Container(
          decoration: BoxDecoration(
            color: isHovering 
                ...
                : (isToday ? const Color(0xFFFFFDE7) : Colors.transparent),
                ...
                child: Padding(
                  ...
                  child: Consumer<BujoProvider>(
                    builder: (ctx, provider, _) {
                      ...
                      return Column(
                        children: dayBullets.map((b) => _buildDraggableStrip(b)).toList(),
                      );
                    },
                  ),
                ),
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            border: isDragging ? Border.all(color: Colors.blue) : null,
          ),
          child: Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
