...
import '../models/collection.dart'; // 引入 Collection 以获取集子名称
import 'bullet_edit_dialog.dart';
import 'bujo_date_picker.dart'; // 【新增】引入，为了使用 calculateWeekNumber
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return ListTile(
          ...
          onTap: () {
            // 点击弹出编辑框，并关闭搜索（或者保持搜索？通常关闭体验较好，或者直接弹窗）
            // 这里直接弹窗，编辑完后搜索结果会实时更新（因为是 Provider）
  ...
  String _getMetaInfo(Bullet b, BujoProvider provider) {
    ...
    if (b.date != null) {
      switch (b.scope) {
        ...
        default:
          scopeStr = "收集箱"; // 有日期但 scope 为 none 的情况
          break;
      }
    }
